            ...
            <div class="p-8 lg:p-10">
                ...
                <div id="state-upload">
                    ...
                    <div id="drop-zone"
                        ...
                        <p class="text-fn-text3 text-sm mb-6">Supports .docx and .doc — or click to browse from your
                            computer</p>
                        <div
                ...
                <div id="state-converting" class="hidden text-center py-6">
                    ...
                    <div class="max-w-xs mx-auto flex flex-col gap-3 text-left">
                        @foreach([
                        ['proc-1','Uploading & converting Word to PDF'],
...
<script>
  ...
  function handleFile(file) {
    hideError();
    if (!['application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document'].includes(file.type) && !file.name.toLowerCase().match(/\.docx?$/)) {
  ...
  async function startConversion() {
    ...
    try {
      const res = await fetch('https://api.filenewer.com/api/tools/word-to-jpg', {
        method: 'POST',
        body:   formData,
      });
