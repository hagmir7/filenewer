        ...
        <div class="bg-fn-surface border border-fn-text/8 rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div class="p-8 lg:p-10">
                ...
                <div id="state-upload">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            <div class="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none">
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" class="text-fn-red">
                            ...
                            <input type="url" id="yt-url" autocomplete="off" spellcheck="false"
                                ...
                                class="w-full bg-fn-surface2 border border-fn-text/10 text-fn-text text-sm rounded-xl pl-10 pr-24 py-3.5 focus:outline-none focus:border-fn-red/40 placeholder:text-fn-text3/50 transition-colors" />
                            <button type="button" id="btn-paste-url"
                                class="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1.5 px-3 py-1.5 bg-fn-surface border border-fn-text/10 text-fn-text3 hover:text-fn-text text-xs font-semibold rounded-lg transition-all">
                        ...
                        <div class="flex items-center justify-between mt-1.5">
                            <span id="url-status" class="text-xs text-fn-text3">Supports youtube.com, youtu.be, Shorts, and embed URLs</span>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-fn-surface2 border border-fn-text/8 rounded-xl mb-5">
                        ...
                        <div class="grid sm:grid-cols-2 gap-3">
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div>
                                    <label for="opt-language" class="text-xs font-semibold text-fn-text2 block mb-1.5">Transcript language</label>
                                ...
                                <div>
                                    <label for="opt-translate" class="text-xs font-semibold text-fn-text2 block mb-1.5">
                                        Translate to
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-1.5">
                                        ...
                                        <label id="{{ $fid }}-wrap"
                                            ...
                                            <input type="radio" name="output_format" value="{{ $fval }}" {{ $fdefault ? 'checked' : '' }} class="sr-only fmt-radio" />
                                            <span class="text-base shrink-0">{{ $ficon }}</span>
                                            <div class="min-w-0">
                                                <p class="text-xs font-bold text-fn-text leading-none">{{ $flabel }}</p>
                                                <p class="text-xs text-fn-text3 leading-tight mt-0.5">{{ $fdesc }}</p>
                                            </div>
                                        </label>
                                        @endforeach
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-col gap-1.5">
                                        @foreach([
                                            ['opt-timestamps', 'Include timestamps', 'Add [HH:MM:SS] before each line (text only)', false],
                    ...
                    {{-- Error banner --}}
                    <div id="upload-error"
                        class="hidden mt-4 mb-4 items-center gap-3 px-4 py-3 bg-fn-red/8 border border-fn-red/25 rounded-xl text-sm text-fn-text2">
                    ...
                    <button id="convert-btn" type="button" disabled
                        class="w-full py-3.5 bg-fn-red text-white font-bold text-base rounded-xl transition-all disabled:opacity-40 disabled:cursor-not-allowed hover:enabled:opacity-90 hover:enabled:-translate-y-0.5 flex items-center justify-center gap-2"
                        style="background: oklch(52% 0.22 25);">
                        <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                        </svg>
                        <span id="fetch-btn-label">Get Transcript</span>
                    </button>
                </div>
                ...
                <div id="state-converting" class="hidden text-center py-6">
                    <div class="flex items-center justify-center gap-5 mb-8">
                        <div class="w-16 h-16 rounded-2xl flex items-center justify-center" style="background: oklch(52% 0.22 25 / 12%); border: 1px solid oklch(52% 0.22 25 / 25%)">
                            <svg width="28" height="28" viewBox="0 0 24 24" fill="currentColor" style="color: oklch(52% 0.22 25)">
                                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                            </svg>
                        </div>
                    ...
                    <h2 class="text-xl font-bold mb-2" id="fetching-title">Fetching Transcript…</h2>
                    <p class="text-fn-text3 text-sm mb-8" id="converting-subtitle">Connecting to YouTube &amp; extracting captions</p>
                    ...
                    <div id="whisper-notice" class="hidden max-w-sm mx-auto mt-6 p-3 bg-fn-amber/6 border border-fn-amber/20 rounded-xl flex items-start gap-2 text-left">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-fn-amber shrink-0 mt-0.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                        <p class="text-xs text-fn-text2 leading-relaxed">Captions not found — falling back to <span class="font-semibold">Whisper AI</span> transcription. This may take a minute…</p>
                    </div>
                </div>
                ...
                <div id="state-download" class="hidden py-2">
                    ...
                    {{-- Video info bar --}}
                    <div class="flex items-start gap-3 mb-5 p-4 bg-fn-surface2 border border-fn-text/8 rounded-xl">
                        <div class="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style="background: oklch(52% 0.22 25 / 12%); border: 1px solid oklch(52% 0.22 25 / 22%)">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style="color: oklch(52% 0.22 25)">
                                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                            </svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="font-bold text-sm truncate" id="video-title">—</p>
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1" id="video-meta-chips"></div>
                        </div>
                        <a id="video-link" href="#" target="_blank" rel="noopener"
                            class="shrink-0 flex items-center gap-1 px-2.5 py-1.5 bg-fn-surface border border-fn-text/10 text-fn-text3 hover:text-fn-text text-xs font-semibold rounded-lg transition-all">
                    ...
                    <pre id="transcript-output"
                        class="w-full bg-fn-surface2 border border-fn-text/10 rounded-xl px-5 py-4 text-sm text-fn-text2 leading-relaxed overflow-auto max-h-[32rem] whitespace-pre-wrap font-sans"></pre>
                    ...
                    <div class="flex items-center justify-between mt-3 flex-wrap gap-2">
                        ...
                        <p class="text-fn-text3 text-xs flex items-center gap-1.5">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
...
<section class="py-12 border-t border-fn-text/7 bg-fn-surface2">
    <div class="max-w-3xl mx-auto px-6">
        ...
        <p class="text-fn-text3 text-sm text-center mb-8">Two-strategy approach for maximum coverage</p>
        <div class="grid sm:grid-cols-2 gap-4">
            <div class="p-5 bg-fn-surface border border-fn-text/8 rounded-2xl">
                <div class="flex items-center gap-2.5 mb-3">
                    <div class="w-9 h-9 rounded-xl bg-fn-green/10 border border-fn-green/20 flex items-center justify-center text-lg">⚡</div>
                    <div>
                        <p class="font-bold text-sm">Strategy 1 — Transcript API</p>
                        <p class="text-xs text-fn-green font-semibold">Fast · ⭐⭐⭐⭐</p>
                    </div>
                </div>
                <p class="text-xs text-fn-text2 leading-relaxed mb-3">Directly fetches the caption track embedded by the video creator or auto-generated by YouTube. Instant — no audio processing needed.</p>
                <div class="flex flex-col gap-1 text-xs text-fn-text3">
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><polyline points="20 6 9 17 4 12"/></svg> No extra dependencies</div>
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><polyline points="20 6 9 17 4 12"/></svg> Language selection & translation</div>
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><polyline points="20 6 9 17 4 12"/></svg> Supports auto-generated captions</div>
                </div>
            </div>
            <div class="p-5 bg-fn-surface border border-fn-text/8 rounded-2xl">
                <div class="flex items-center gap-2.5 mb-3">
                    <div class="w-9 h-9 rounded-xl bg-fn-purple/10 border border-fn-purple/20 flex items-center justify-center text-lg">🎙️</div>
                    <div>
                        <p class="font-bold text-sm">Strategy 2 — Whisper AI</p>
                        <p class="text-xs text-fn-purple font-semibold">Slower · ⭐⭐⭐⭐⭐</p>
                    </div>
                </div>
                <p class="text-xs text-fn-text2 leading-relaxed mb-3">Falls back to downloading the audio and running OpenAI Whisper for AI-powered speech-to-text when no captions are available.</p>
                <div class="flex flex-col gap-1 text-xs text-fn-text3">
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><polyline points="20 6 9 17 4 12"/></svg> Works on any video, even without captions</div>
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="text-fn-green"><polyline points="20 6 9 17 4 12"/></svg> Highest transcription accuracy</div>
                    <div class="flex items-center gap-1.5"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-fn-amber"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> Takes longer — audio download + AI processing</div>
                </div>
            </div>
        </div>
    </div>
...
<section class="py-12 border-t border-fn-text/7 bg-fn-surface">
    <div class="max-w-3xl mx-auto px-6">
        ...
        <p class="text-fn-text3 text-sm text-center mb-6">Choose the format that fits your workflow</p>
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div class="p-4 bg-fn-surface2 border border-fn-text/8 rounded-xl">
                ...
                <p class="text-xs text-fn-text3 mt-1.5 leading-relaxed">{{ $desc }}</p>
            </div>
            @endforeach
        </div>
    </div>
...
{{-- ══ FAQ ══ --}}
<section class="py-16 border-t border-fn-text/7 bg-fn-surface2">
    <div class="max-w-3xl mx-auto px-6">
        <h2 class="text-2xl font-bold tracking-tight mb-8 text-center">Frequently Asked Questions</h2>
        <div class="space-y-3">
            @foreach([
                ['Which YouTube URL formats are supported?', 'Standard watch URLs (youtube.com/watch?v=...), short URLs (youtu.be/...), Shorts (youtube.com/shorts/...), embed URLs (youtube.com/embed/...), and even bare URLs without the protocol (youtube.com/watch?v=...) are all accepted.'],
                ['What if the video has no captions?', 'If no caption track is found, the tool automatically falls back to Whisper AI — it downloads the audio and runs OpenAI\'s speech-to-text model. This produces a high-quality transcript for almost any video, though it takes longer than caption extraction.'],
                ['Can I get transcripts in languages other than English?', 'Yes — select the transcript language to fetch an existing caption track in that language. You can also translate the transcript to another language after fetching it by selecting a target in the "Translate to" field.'],
                ['What is the difference between SRT and VTT?', 'SRT (SubRip) is the most widely supported subtitle format — compatible with VLC, Premiere Pro, DaVinci Resolve, and most video tools. VTT (WebVTT) is the standard for HTML5 video elements and browser-based players. Both contain timed captions; the syntax differs slightly.'],
                ['What does auto-captions do?', 'When enabled (default), the tool will also check for YouTube\'s automatically generated captions if no manual captions are found in the requested language. Auto-captions are generally accurate for clearly spoken English and many other languages.'],
                ['Is the transcript stored or logged?', 'No. The transcript content is returned directly to your browser and is never stored, indexed, or shared. It is cleared when you close or refresh the page.'],
            ] as [$q, $a])
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  urlInput.addEventListener('input', () => {
    ...
    if (vid) {
      status.innerHTML = `<span class="text-fn-green flex items-center gap-1.5">
        ...
        Valid YouTube URL · Video ID: <code class="font-mono">${vid}</code>
      </span>`;
  ...
  document.querySelectorAll('.fmt-radio').forEach(radio => {
    radio.addEventListener('change', () => {
      ...
      // Disable timestamps for non-text formats
  ...
  async function startFetch() {
    ...
    document.getElementById('converting-subtitle').textContent =
      `Fetching ${outputFormat.toUpperCase()} transcript · language: ${language}${translateTo ? ' → ' + translateTo : ''}`;
    ...
    const t3 = setTimeout(() => {
      ...
      // Show Whisper notice after 8s if still loading
    }, 1600);
    const t4 = setTimeout(() => {
  ...
  function showResult(data, format, url) {
    ...
    chips.innerHTML = '';
    const metas = [];
    if (data.duration_str)   metas.push(['🕐', data.duration_str]);
    if (data.word_count)     metas.push(['📝', Number(data.word_count).toLocaleString() + ' words']);
    if (data.segment_count)  metas.push(['📋', data.segment_count + ' segments']);
    if (data.language)       metas.push(['🌐', data.language.toUpperCase()]);

    metas.forEach(([icon, label]) => {
    ...
    document.getElementById('result-stats').textContent =
      data.char_count ? `${Number(data.char_count).toLocaleString()} characters` : '';

    const ext = extMap[format] || 'txt';
    document.getElementById('download-ext-label').textContent = `Download .${ext}`;
    const safeTitle = (data.video_id || 'transcript').replace(/[^a-z0-9]/gi, '_').toLowerCase();
    lastFilename   = `${safeTitle}_transcript.${ext}`;
  ...
  function showState(state) {
    ...
    // Reset whisper notice on any state change
