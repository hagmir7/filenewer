...
<body>
    ...
    <div class="doc-header">
        <div class="doc-title">{{ $settings['custom_title'] }}</div>
        <div class="doc-meta">
            {{ count($sheets) }} sheet(s) &middot;
            {{ collect($sheets)->sum(fn($s) => max(0, count($s['rows']) - 1)) }} data rows
        </div>
        <div class="doc-date">Generated: {{ now()->format('d M Y, H:i') }}</div>
    </div>
